...
`git pull` 会因为工作簿改过而拒绝,或者更糟 —— **`.xlsx` 是二进制,合不了**,
两边都改过就只能二选一。所以拉之前先安顿:
...
`序 | Unit | Name | From | 至 | 关系 | Remark | Source`

- **Unit** 是这家单位**如今的正名**,一家单位的几行都写同一个 —— 它是钥匙,不是那一段的名字
- **Name** 才是那一段时间里它叫什么
- **序** 是同一家单位里的第几个名字,**至** 是这个名字用到哪一年(下一段启用那年);
  末一段的「至」空着,那就是现在的名字
