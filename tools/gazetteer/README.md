...
从头到尾的流程写在一份文档里,是给 Obsidian 读的(Mermaid 流程图、
callout、可勾的清单):

- [`电子工业地图流程.md`](电子工业地图流程.md) —— 从一份 .md 的篇章,到线上那张图更新:
  放稿、抽取、**核对**、并表、验、落点、合进 main。
...
底下这份 README 是工具的说明书;要问「这一步该干什么」,看《电子工业地图流程》。
...
| 表 | 作什么用 | 改了算不算数 |
| --- | --- | --- |
| 待核·厂所 | 厂所,核对与改字都在这儿 | **算** |
| 待核·器件 / 待核·整机 / 待核·名称沿革 | 器件、整机、名称沿革 | **算**(各有「取否」列) |
| 预览·照总表体例(改它不算数) | 照原表体例生成,好看出将来落在地图上是什么样 | **不算** —— 它是待核那几张的另一种排法,读回来只读冠「待核·」的 |

**要核的是冠着「待核·」的那四张**,一张漏了那一张就整张不进总表。从前头一张
就叫「待核」,另三张挂着总表的名字(`器件`、`整机`、`名称沿革`),看着像成品;
四张一律冠「待核·」,一眼看出是一套。**旧本子照样读得动** —— 历来用过的标签
一律认。

**重跑一本核过的志书,还多一张 `待核·补全已有记录`。** 那些单位大半早已在总表里
(`待核·厂所`那张给它们标上「已收」),`append` 一概跳过 —— 跳过的同时也就把
这一遍新认出来的隶属、性质一并扔了。补全已有记录那张补的正是这一处:一行一格,
`取否` 写 `y` 的才填,只动那几格,一行也不新增。核过一遍的志书重跑,要核的
从「一百多家单位」变成「一百来个格子」。

**「待核·厂所」就是你落笔的地方。** 每家一行,前五列是 `取否 | 单位 | 置信 |
出处 | 据以立论的原文`,冻在第 C 列 —— 核的是名字,名字与据以判断的原文
并排看,不必横拉。名字认错的当场改,要的行「取否」写 `y`,核完:
