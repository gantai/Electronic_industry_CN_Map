...
```powershell
npm run dev                                        # 本地看图
$vault = "D:\...\你的库\厂所"     # 库里的一个子文件夹,不是库根
python tools\gazetteer\gaz.py push --vault $vault   # 工作簿 → 笔记
python tools\gazetteer\gaz.py pull --vault $vault   # 笔记 → 工作簿
```
...
> ```powershell
> Get-ChildItem D:\ -Directory -Filter ".obsidian" -Recurse -Depth 4 -ErrorAction SilentlyContinue |
>   ForEach-Object { $_.Parent.FullName }
> ```
> 省得每回重打:`[Environment]::SetEnvironmentVariable("GAZ_VAULT", "…\你的库\厂所", "User")`
> —— `push`/`pull`/`guide` 不给 `--vault` 时都认它。
>
