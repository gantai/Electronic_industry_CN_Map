...
flowchart TD
    ...
    E -->|npm run dev| F["本地看图"]
    E -->|合进 main| H["GitHub Pages<br/>线上那张图"]
...
设完**新开一个终端** —— 环境变量与 PATH,都只有新起的程序看得见。
...
> (`deploy.yml` 里写着 `node-version: 20`)。所以本机没有 Node,
> 照样合得进 main,线上照样更新;只是合之前没法自己先看一眼。
...
`.github/workflows/deploy.yml` 只认一件事:**main 上有新提交**。别的分支
推得再勤,线上那张图一动不动 —— 它压根不会为分支跑构建。

上线前先过一遍关卡(**这一步别省** —— 推进 main 就是发布,退回来要另发一次):
...
`npm run dev` 起来自己看一眼:数目对不对、点落在该落的地方、改名的年份
像不像话。**改过工作簿要重启 dev**,光刷新页面看的还是旧的。

(没装 Node 就跳过这一句 —— 前两句照跑,合进 main 一样上线。)
...
推上去之后,GitHub 的 **Actions** 页上会有一条 "Deploy to GitHub Pages"
在跑,两三分钟。绿了,线上就是新的。

> [!tip] 也可以在 GitHub 上开 Pull Request 合
> 网页上看得见改了哪些文件,合之前还能回头。命令行这一路快些,
> 结果一样 —— 都是往 main 上落一个提交。

> [!warning] main 上的东西是公开的
> 合进去的不只是工作簿:仓库里那一刻有什么,构建就带什么。转换稿要是
> 没挡在 `.gitignore` 外头(见第一步),整本志的誊录稿会跟着上线。

> [!note] 最后一次合进 main 之后又攒了多少
> ```powershell
> cd D:\Coding\CN_Map
...
- [ ] **`git pull`** —— 别拿旧版跑
- [ ] 关掉 Excel
- [ ] 转成 Markdown(或确认已有),**挪进 `D:\Coding\CN_Map\转换稿`**
...
- [ ] `gaz verify` 验一验:名录连得上吗、年份、坐标、出处
- [ ] `git commit` 留下修订史(**没设 user.name 会拒绝提交**)
- [ ] **重启** `npm run dev` 看图上对不对 —— 工作簿是编译进去的,不重启看不见
- [ ] 要上线得**合进 main**(见《上线》):GitHub Pages 只认 main,
      分支推多少次都不会更新;合完看 Actions 那条 Deploy 跑绿了没有
- [ ] `gaz push --vault` 推进库里
