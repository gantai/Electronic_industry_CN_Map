...
const zh = {
  ...
  provNone: (p) => `${p} · 尚未收录`,
  provHeld: (p, n) => `${p} · 收了 ${n} 家,年份或地址还没考出来`,
  legendRecords: "记录",
  showableOnly: "只放齐全的",
  showableHint: "始建年与街/区级地址都考出来的,才在时间轴上动得起来",
  showablePut: (n) => `另有 ${n} 家年份或地址还没考出来,暂不上图`,
  showableAll: (n) => `全部 ${n} 家都在图上,含年份或地址未详的`,
  barsUndated: "无始建年",
  barsNote: "彩色一截随年份涨落;灰的一截是志书没写始建年的,压在底下不动",
  barsNoteClean: "柱子随年份涨落 —— 眼下只放年份与地址都考出来的那一批",
...
const en = {
  ...
  provNone: (p) => `${p} · nothing on record yet`,
  provHeld: (p, n) => `${p} · ${n} on record, still lacking a year or an address`,
  legendRecords: "RECORDS",
  showableOnly: "Complete only",
  showableHint: "A unit needs both a founding year and a street- or district-level address to move on the timeline",
  showablePut: (n) => `${n} more still lack a year or an address and are held back`,
  showableAll: (n) => `all ${n} shown, including those with no year or address`,
  ...
  barsNoteClean: "Bars follow the year — showing only units with both a founding year and an address",
  barTip: (p, live, und, tot, y) =>
